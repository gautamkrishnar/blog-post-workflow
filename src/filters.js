...
const COMMENT_FILTERS = core
	...
	.map((item) => {
		if (
			item.trim().startsWith('stackoverflow') ||
			item.startsWith('stackexchange')
		) {
...
const ignoreStackOverflowComments = (item) =>
	!(
		...
		item.title.startsWith(
			FILTER_PARAMS.stackoverflow.replace(/\$author/g, item.author),
		)
	);
...
const ignoreStackExchangeComments = (item) =>
	!(
		...
		item.title.startsWith(
			FILTER_PARAMS.stackexchange.replace(/\$author/g, item.author),
		)
	);
